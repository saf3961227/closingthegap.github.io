 ...
 */
public class CTGProcessCSVHealthConditions2021 {
      ...
      try {
         ...
         while (lineScanner.hasNext()) {
            ...
            while (rowScanner.hasNext() && indexCategory < category.length) {
               ...
               String query = "INSERT into HealthCondition VALUES ("
                              + "'" + status[indexStatus] + "',"
                              + "'" + category[indexCategory] + "',"
                              + "'" + sex[indexSex] + "',"
                              + count + ","
                              + lgaCode + ","
                              + "2021" + ")";  //hardcoding year for 2021 lgas
               ...
               if (indexSex >= sex.length) {
                  ...
                  if (indexStatus >= status.length) {
                     // Go to next Category
                     rowScanner.next();
                     rowScanner.next(); //skipping the non-stated
                     indexStatus = 0;
                     indexCategory++;
                  }
               }
